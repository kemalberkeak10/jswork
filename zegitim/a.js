$(function() {
    ...
    $('body').on('click', '#sertifikaIndir', function() {
        if (calibratedTechnicianId === "" || theScopeResponsibleId === "" || headOfCalibrationId === "") {
            ...
            var localUrl = String.format("http://localhost:65474/api/data/CertificateTemplateWithDevExpress?recordId={0}&dokuman={1}", $('#RecordPublicId').val(), 'ThyWord.docx');
            var url = String.format("https://thywebapi.setcrm.com/api/data/CertificateTemplateWithDevExpress?recordId={0}&dokuman={1}", $('#RecordPublicId').val(), 'ThyWord.docx');
            $.get(url, function(r) {
