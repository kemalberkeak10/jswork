$(function() {
    ...
    $('body').on('click', '#btnKaydetQualified', function() {
        ...
        $.post(url, model, function(r) {
            if (r.Status) {
                var realUrl = 'https://saphirewebapi.setcrm.com/api/data/ContactLastFollowupUptade?recordId=' + $('#RecordPublicId').val();
                $.get(realUrl, function(r) {
                        if (r.Status) {
                            $('.btn-br-actions[data-publicid=68C88B96C3CE4A4E913143F0C264371A]').trigger('click');
                        } else {
                            notify("danger", "Last Follow-Up Date Güncellenirken Hata Oluştu");
                        }
                    })
                    //$('#modalQualified').modal('toggle');
            } else {
