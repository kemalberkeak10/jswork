$(function() {
    if ($('#ViewFilterPublicId').val() === "C24126D50EB14979B8EEA13167260C12" || $('#ViewFilterPublicId').val() === "574A986BBA8645FC9A04999232F65283") {
        ...
        $('.well .pull-right:eq(0)').prepend('<a id="btnCalibrasyonToinfoLocation" class="btn btn-sm btn-warning"  style="margin-right:10px;" title=""><span>Traxdan Uygulamaya Aktar</a>');
        var model = {};
        ...
        $('body').on('click', '#btnLocationSearch', function() {
            if ($('#traxLocation').val() === "") {
                ...
                $('#txt').show();
                var localUrl = String.format('http://localhost:65474/api/data/CalibrationToCreateInfoLocation?location={0}', $('#traxLocation').val().toUpperCase()),
                    realUrl = String.format('https://thywebapi.setcrm.com/api/data/CalibrationToCreateInfoLocation?location={0}', $('#traxLocation').val().toUpperCase());
