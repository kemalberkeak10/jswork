jQuery(function ($) {
  //add user with enter
  $("body").on("keyup", "#addUserInput", function (e) {
  ...
  new Draggable(containerEl, {
    ...
    eventData: function (eventEl) {
      return {
        ...
        backgroundColor: document.getElementById(eventEl.id).style
          .backgroundColor,
      };
    },
  });
...
function newElement() {
  ...
  if (inputValue.trim() === "") {
    ...
    var newUserDiv =
      `<div class="card text-white mb-3 mt-2" id="card-` +
      cardIndex +
      ...
      cardIndex +
      `">
            <i class="fa fa-user-md" style=font-size:2em;>   ` +
      inputValue +
      `</i> 
        </div>
        </div>`;
...
function createUserDragElements() {
  ...
  $.each(userList, function (i, v) {
    ...
    var newDragItem =
      `<div class="fc-event card text-white mb-3 mt-2" id="card-` +
      v.id +
      `" style="background-color:` +
      cardColorCode +
      `">
        <div class="fc-event-main card-body" id="card-body-` +
      v.id +
      `"><i class="fa fa-user-md" style=font-size:2em;>   ` +
      v.name +
      `</i></div></div>`;
...
function shiftDayCount() {
  var resmiTatiller = $("#secondCalendar tbody tbody tr").find(
    ".fc-event-start"
  );
  var resmiTatilCount = $("#secondCalendar tbody tbody tr").find(
    ".fc-event-start"
  ).length;
