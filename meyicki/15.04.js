$(function() {
    ...
    var barkodIsChecked = $('label[for=DBF500A7E6684E9E85F1D8079DAEF897]').parent().data('value');
    console.log(barkodIsChecked);
    barkodIsChecked = String.isNullOrWhiteSpace(barkodIsChecked) ? false : toBool(barkodIsChecked);
    $('body').on('click', '#btnSatisOnayy', function() {
            ...
            window.setModal.Create({
                ...
                html: {
                    ...
                    body: '<div id="msg" style="margin:0 23px 5px; width: 80%;font-size:15px;"></div>' +
                        ...
                        '</tbody>' +
                        '</table>',
                    footer: '<button id="btnKaydetSatis" type="button" class="btn btn-sm btn-success" >Kaydet ve Onayla</button>' + '<button type="button" class="btn btn-sm btn-default" data-dismiss="modal">Kapat</button>'
                },
                settings: {
                    widthClass: 'modal-lg'
                }
        ...
        } else if (!barkodIsChecked) {
            $('#modalSablon').remove();
            window.setModal.Create({
                id: 'modalSablon',
                html: {
                    content: 'style="width:900px height:150px !important"',
                    header: 'Barkod Basılmadı',
                    body: '<div id="msg"></div><div id="txt" style="margin:0 0 5px; width: 100%;">İşleme Devam Etmek İçin Barkod Basmanız Gerekiyor<br/></div>',
    ...
    $('body').on('click',
        ...
        function() {
            ...
            var url2 = 'http://localhost:11456/api/data/SatisOnayGuncelle';
            ...
            $.post(url, data, function(r) {
                if (r.Status) {
                    $('.btn-br-actions[data-publicid=AE3A3FCDA8CF460CACA7CF2ACC88DE38]').trigger('click');
                    $('#modalSablon').modal('toggle');
                    setTimeout(() => {
                        recordId = $("#RecordPublicId").val();
                        var urlReal = 'https://meyicki.setcrm.com/api/data/SatisOnayKaydet';
                        var urlLocal = 'http://localhost:11456/api/data/SatisOnayKaydet';
                        $.get(urlReal, {
                                recordId: recordId,
                            ...
                            function(r) {
                                if (r.Status) {
                                    ...
                                    $('#modalSablon').modal('toggle');
                                    $('#modalSablon .modal-body').html(r.Message);
                                }
                            });
                    }, 500);
                } else {
