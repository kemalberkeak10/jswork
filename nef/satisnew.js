$(function() {
    ...
    $('body').on('change', '#AA72140A2F304FAF822443E030335221', function() {
        musteriId = $(this).select2('data').id;
        if (String.isNullOrWhiteSpace(musteriId)) return;

        if ($('#modalMusteri').length > 0) return;
        // if ($('#s2id_AA72140A2F304FAF822443E030335221').is('.select2-container-disabled')) {
        ModalCreate();
        $('#txt').show();
    ...
    window.setModal.Create({
        ...
        html: {
            ...
            body: '<div id="musteriMsg"></div>' +
                ...
                '<tbody>' +
                '<tr>' +
                '<td><input class="form-control" id="tcNo" type="number" style="resize: vertical; width:100%;"></input></td><td><input class="form-control" id="uyruk" type="text" style="resize: vertical; width:100%;"></input></td>' +
                '</tr>' +
                '</tbody>' +
                '</table>' +
                '<table class="table" style="width: 100%">' +
                ...
                '<tbody>' +
                '<tr>' +
                '<td style="width:500px; height:40px;"><input class="form-control" id="egitimDurumu" type="text" style="resize: vertical; width:100%;"></input></td><td style="width:500px; height:40px;"><div id="cinsiyet" class="cinsiyet"></div></td>' +
                '</tr>' +
                '</tbody>' +
                '</table>' +
                '<table class="table" style="width: 100%">' +
...
function checkValidationMusteri() {
    if (String.isNullOrWhiteSpace($('#meslek').select2('data')) || String.isNullOrWhiteSpace($('#tcNo').val()) || String.isNullOrWhiteSpace($('#egitimDurumu').val()) || String.isNullOrWhiteSpace($('#uyruk').val()) ||
