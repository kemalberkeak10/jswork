$(function() {
    ...
    setTimeout(function() {
        ...
        if ($('#FilterId').val() === '0EA243DAD5134BB0AAA6E2103F812DA5' || $('#FilterId').val() === '8031D291FD42486CBD0F18E73C3D334C' || $('#FilterId').val() === '0B5F9F7E9B4C45A28D99A5E7860E89FC') {
            $(".well .pull-right:eq(0)").prepend(
                '<a id="eimzaOlusturETugra" class="btn btn-sm btn-success " >E-imza Programına Gönder Yeni</a>'
            );
            var imzaBilgiler = [];
            $(".table-bordered")
                .find("tbody tr")
    ...
    $("body").on("click", "#eimzaOlusturETugra", function() {
        var sayi = $(".table-bordered tbody tr").find(
            "td:eq(0) input:checked"
        ).length;
        if (sayi > 7) {
            ...
            if ($("#FilterId").val() === "8031D291FD42486CBD0F18E73C3D334C") {
                ...
                $(".table-bordered tbody tr")
                    .find("td:eq(0) input:checked")
                    .each(function() {
                        var satirId = $(this).closest("tr").data("id");
                        // imzalar.push($(this).closest('tr').data('id'));
                        $.get(
                            url +
                            "/api/data/LabMuduruKontrol?recordId=" +
                            $(this).closest("tr").find("td:eq(7)").data("value") +
                            "&tip=" +
                            $(this).closest("tr").find("td:eq(4)").data("value"),
                            function(result) {
                                if (result !== "islem devam edebilir") {
                                    ...
                                    if (imzalar.length === sayi) {
                                        ...
                                        window.open(
                                            url +
                                            "/api/templatelotus/TopluImzalaETugra?imzalar=" +
                                            imzalar
                                        );
                                    }
                                }
                ...
                data.ImzaBilgiler = satirBilgiler;
                $.post(
                    url + "/api/data/EimzaSurecSorgulaLab?data=",
                    data,
                    function(result) {
                        if (result.length > 0) {
                            for (var i = 0; i < result.length; i++) {
                            ...
                            window.open(
                                url + "/api/templatelotus/TopluImzalaETugra?imzalar=" + imzalar
                            );
                        }
                    }
                );
                ...
                data.ImzaBilgiler = satirBilgiler;
                $.post(
                    url + "/api/data/EimzaSurecSorgula?data=",
                    data,
                    function(result) {
                        if (result.length > 0) {
                            for (var i = 0; i < result.length; i++) {
