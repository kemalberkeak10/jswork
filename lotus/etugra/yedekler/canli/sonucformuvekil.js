    ...
    if ($('#LayoutPublicId').val() === '9374D4370BAF48F58536C854F817C23F') {
        ...
        $.get('/set/list/personel-analiz-yetkilendirme/?filter=7C8AF72CE4B54EB5A743BA99A773B063', function(data) {
            ...
            if (parseFloat(musteriToplam) > 0) {
                $('.well .pull-right:eq(0)').prepend('<a id="btnSonucFormuVekileimzaYeni" class="btn btn-sm btn-success" ><i class="fa fa-file-excel-o"/>Vekil İçin Sonuç Bildirim Formu Eimza Gönder Yeni</a>');
            }
        });
        $('body').on('click',
            '#btnSonucFormuVekileimzaYeni',
            function() {
                $.get(url + '/api/data/LabIdBul?user=' + userData.name + '&filterId=' + '3146395084E44DC09ED330989A7745CC', function(result) {
                    if (result.Message !== '') {
                        ...
                        var urls = String.format(url + "/api/TemplateLotus/SonucFormuETugraSon?recordId=" + recordId + '&lab=' + lab);
