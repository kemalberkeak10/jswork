$(function() {
    ...
    $('#modalNumuneBilgisi').remove();
    window.setModal.Create({
        id: 'modalNumuneBilgisi',
        html: {
            header: 'Bilgiler',
            body: '<div id="SubelerMesaj" style="margin:0 0 5px;width: 100%">' +
                ...
                '<tbody></tbody>' +
                '</table></td>' +
                '<table id="tableFirmaNotu" class="table table-striped table-hover">' +
                '<thead>' +
                '<tr>' +
                '<th>Firma Numune Notu</th>' +
                '</tr>' +
                '</thead>' +
                '<tbody></tbody>' +
                '</table>',
    ...
    $('body').on('click', '#numuneBilgi', function() {
        ...
        $.get(url + '/api/data/NumunedenBilgiGetir?ilgiliNumune=' + id, function(result) {
            $('#modalNumuneBilgisi').modal('toggle');
            $('#modalNumuneBilgisi #SubelerMesaj').find('tbody').html('');
            if ($(result.documentElement).html() === 'false') {
                ...
                if (result.Message === '') {
                    ...
                    $.each(result.Message.split(','), function(i, v) {
                        ...
                        var subNewRow = $('<tr class="trdetay_' + q + '"/>');
                        ...
                        subNewRow.append($('<td data-id=""/><input style="display: none;">').text(v.split('|')[2]));
                        $('#tableSubeler').find('tbody').append(newRow);
                        $('#tableFirmaNotu').find('tbody').append(subNewRow);
                        q = q + 1;
                    });
                }
            }
